...
export default function SeoPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data } = await supabase.from('seo_audit').select('*').order('data_audit', { ascending: false }).limit(10)
  ...
  async function avviaAudit() {
    ...
    try {
      ...
      const res = await fetch(`${base}/workflow-L`, {
        ...
        body: JSON.stringify({ sito_url: url, periodo, model: aiModel, openrouter_key: orKey || undefined }),
      })
